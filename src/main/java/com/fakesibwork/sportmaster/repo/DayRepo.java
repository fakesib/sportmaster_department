...
public interface DayRepo extends JpaRepository<Day, Integer> {
    ...
    @Modifying
    @Query(value = "INSERT INTO day(id, chat, name, store, role, delivery, here, mobile, email, fast) values(DEFAULT, ?1, ?2, ?3, ?4, 0, 0, 0, 0, 0)", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "INSERT INTO day(store, chat, name, role, delivery, email, fast, here, mobile) SELECT store, chat, name, role, 0 AS delivery, 0 AS email, 0 AS fast, 0 AS here, 0 AS mobile FROM users WHERE role = 'STORE'", nativeQuery = true)
    void addStores();

    @Modifying
    @Query(value = "INSERT INTO day(store, chat, name, role, delivery, email, fast, here, mobile) SELECT store, chat, name, role, 0 AS delivery, 0 AS email, 0 AS fast, 0 AS here, 0 AS mobile FROM users WHERE role = 'ADMIN'", nativeQuery = true)
    void addAdmins();
